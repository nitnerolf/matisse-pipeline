...
def fluxcal(inputfile_sci, inputfile_cal, outputfile, dir_caldatabases, 
    ...
    if mode == 'corrflux' or mode == 'both':
        ...
        for j in range(len(inhdul_sci['OI_VIS'].data['VISAMP'])):
            ...
            diam_err_cal_rad = diam_err_cal/1000.0/3600.0*math.pi/180.0
            ...
            corrfluxerr_calibrated_sci = corrflux_calibrated_sci*np.sqrt((corrfluxerr_raw_sci/corrflux_raw_sci)**2 + (corrfluxerr_raw_cal/corrflux_raw_cal)**2 + (vis_err_cal/vis_cal)**2)
            if 'L' in band:
